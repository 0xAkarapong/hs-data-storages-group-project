import sys
from concurrent.futures import ThreadPoolExecutor
from decimal import Decimal
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from sqlalchemy import func, select

from db import BusinessError, create_tables, init_engine, tx
from models import Payment
from operations.auth import register_user
from operations.betting import place_bet, settle_outcome
from operations.streaming import start_streaming_session
from seed import seed_demo_flow


def run(fn, n):
    with ThreadPoolExecutor(max_workers=n) as ex:
        futs = [ex.submit(fn, i) for i in range(n)]
    ok, err = [], []
    for f in futs:
        try:
            ok.append(f.result())
        except BusinessError as e:
            err.append(str(e))
    return ok, err


if __name__ == "__main__":
    init_engine()
    create_tables(drop_first=True)
    ctx = seed_demo_flow()

    # A) 20 threads, same email → exactly 1 succeeds
    ok, err = run(lambda _: register_user("dup@example.com", "pw", "Dup"), 20)
    print("register:", len(ok), "created /", len(err), "rejected")     # 1 / 19
    assert len(ok) == 1 and len(err) == 19

    # B) Standard plan caps at 2 streams; 1 is open → 10 threads, 1 more allowed
    ok, err = run(lambda _: start_streaming_session(ctx["user_id"], ctx["event_id"]), 10)
    print("streams:", len(ok), "opened /", len(err), "rejected")        # 1 / 9
    assert len(ok) == 1 and len(err) == 9

    # C) Same idempotency key 8x → one bet, one payment
    ok, err = run(lambda _: place_bet(ctx["user_id"], ctx["outcome_id"],
                                      Decimal("10.00"), None, "bet:dup:99"), 8)
    print("distinct bets:", len({r["bet_id"] for r in ok}))             # 1
    assert not err
    assert len({r["bet_id"] for r in ok}) == 1
    assert sum(not r["replayed"] for r in ok) == 1
    assert sum(r["replayed"] for r in ok) == 7

    # D) 5 concurrent settlements → paid exactly once
    ok, _ = run(lambda _: settle_outcome(ctx["outcome_id"], won=True), 5)
    print("effective settlements:", sum(0 if r["already_settled"] else 1 for r in ok))  # 1
    assert sum(0 if r["already_settled"] else 1 for r in ok) == 1

    with tx() as s:
        duplicate_payments = s.scalar(select(func.count()).select_from(Payment)
                                      .where(Payment.idempotency_key == "bet:dup:99"))
        payout_rows = s.scalar(select(func.count()).select_from(Payment)
                               .where(Payment.idempotency_key.like("payout:%")))
        print("payout rows:", payout_rows)
        assert duplicate_payments == 1
        assert payout_rows == 2
