...
class Outcome(Base):
    ...
    sports_event_id: Mapped[int] = mapped_column(ForeignKey("sports_events.sports_event_id"), nullable=False)
    description: Mapped[str] = mapped_column(String(200), nullable=False)
    status: Mapped[OutcomeStatus] = mapped_column(Enum(OutcomeStatus, name="outcome_status"),
    ...
    __table_args__ = (UniqueConstraint("sports_event_id", "description", name="uq_outcome_desc"),)


class OddsSnapshot(Base):
    __tablename__ = "odds_snapshots"
    snapshot_id: Mapped[int] = mapped_column(primary_key=True)
    outcome_id: Mapped[int] = mapped_column(ForeignKey("outcomes.outcome_id"), nullable=False)
...
class Bet(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.user_id"), nullable=False)
    snapshot_id: Mapped[int] = mapped_column(ForeignKey("odds_snapshots.snapshot_id"), nullable=False)
    streaming_session_id: Mapped[int | None] = mapped_column(ForeignKey("streaming_sessions.session_id"))
    amount_staked: Mapped[Decimal] = mapped_column(MONEY, nullable=False)
    status: Mapped[BetStatus] = mapped_column(Enum(BetStatus, name="bet_status"),
